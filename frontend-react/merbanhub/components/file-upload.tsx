...
//       filesWithProgress.forEach((f) => simulateUpload(f.id))
//     },
//     [files.length, maxFiles, maxFileSize, acceptedFileTypes, onFilesSelected, simulateUpload],
//   )
...
//       {files.length > 0 && (
//         <div className="space-y-2">
//           <h3 className="text-sm font-medium text-foreground">Uploaded Files ({files.length})</h3>
//           <div className="space-y-2">
//             {files.map((fileItem) => {
...
//                         </span>
//                       </div>
//                       {fileItem.status === "uploading" && <Progress value={fileItem.progress} className="h-1.5" />}
    ...
    async (fileToUpload: FileWithProgress) => {
      ...
      try {
        const response = await fetch("http://localhost:8080/api/files/upload", {
          // Adjust URL if your backend is on a different host/port
          method: "POST",
          body: formData,
